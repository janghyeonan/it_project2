...
def dataMake(): # input : 음성파일이 담겨있는 파일경로 ex. "/Users/.../sori.wav"
    # 음성데이터 분석    
    r,sr = librosa.load("C:/inetpub/flask2/tmp/sound.wav", mono=True, sr=16000) # librosa 사용(모노, 샘플레이트 : 16000)
...
## 훈련 및 테스트 데이터셋 세팅 (모델값)
def models():
    x = pd.read_csv("C:/inetpub/flask2/model/x_data.csv", index_col = 0).values
    y = pd.read_csv("C:/inetpub/flask2/model/y_data.csv", index_col = 0).values
    z = dataMake()
